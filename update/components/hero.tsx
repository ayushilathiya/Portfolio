...
export default function Hero() {
  ...
  return (
    <section className="min-h-screen flex flex-col justify-center relative overflow-hidden px-6 md:px-12">
      {/* Status LED strip */}
      <div className="absolute top-6 right-6 md:right-12 flex items-center gap-3">
        <div className="status-led status-led-active" />
        <span className="mono-label text-amber">SYSTEM: ONLINE</span>
      </div>

      {/* Main content */}
      <div className="max-w-5xl mx-auto w-full">
        {/* Role tag above name */}
        <div className="font-mono text-sm md:text-base text-text-muted mb-4 tracking-wider">
          {profile.roleTag}
        </div>
        ...
        {/* Part number style role */}
        <div className="font-mono text-base md:text-lg text-amber mb-8">
          {profile.title}
        </div>
      ...
      <svg
        className="absolute top-0 left-0 w-32 h-32 md:w-48 md:h-48 text-amber opacity-20"
        viewBox="0 0 100 100"
        fill="none"
      >
      ...
      <svg
        className="absolute bottom-0 right-0 w-32 h-32 md:w-48 md:h-48 text-amber opacity-20"
        viewBox="0 0 100 100"
        fill="none"
      >
